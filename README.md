...
Untrusted input is a transaction; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a transaction
- **Closed type:** `type Decision = Flag(Risk) | Clear` where `Risk` is closed
- **Sink / capability:** `grant flag confidence 70` → `privileged { flag(d) }`
- **Consequence axes:** Trust, Confidence

## Run the demo

```sh
examples/apps/fraud-scorer/demo.sh
```
...
- `fraud-scorer_safe.eide` — the correct design.
- `fraud-scorer_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
